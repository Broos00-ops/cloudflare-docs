...
Directly uploading assets via Worker APIs is an advanced approach that most users won't need. Instead, users are encouraged to interact with these APIs through [Wrangler](/workers/static-assets/get-started/#1-create-a-new-worker-project-using-the-cli).
